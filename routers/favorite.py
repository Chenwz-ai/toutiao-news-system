...
async def check_favorite(news_id:int =Query(...,alias="newsId"),
    ...
    is_favorited = await favorite.is_news_favorite(db,news_id,user.id)
    return success_response(message="检查收藏状态成功",data=FavoriteCheckResponse(isFavorite=is_favorited))

@router.post("/add")
async def add_favorite(data:FavoriteAddResponse,
                       user:User = Depends(get_current_user),
                       db:AsyncSession = Depends(get_db)
                       ):
...
async def clear_favorite(
    ...
    return success_response(message=f"取消收藏成功,取消了{count}条收藏")
