...
class Category(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True,autoincrement= True)  #primary_key:是否为主键,autoincrement:是否自增
    name: Mapped[str] = mapped_column(String(50),unique=True,nullable=False, comment="分类名称")   #unique:是否唯一,nullable:是否允许为空
...
class News(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_publish_time','publish_time')        #按发布时间查询
    )
